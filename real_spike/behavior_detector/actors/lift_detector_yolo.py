...
class LiftDetector(ZmqActor):
    def __init__(self, *args, **kwargs):
        ...
        self.frame_num = None
        self.latency = LatencyLogger(name="lift_behavior_detector_yolo",
                                     max_size=20_000,
                                     )
        self.offset = 0
    ...
    def setup(self):
        ...
        self.model = YOLO("/home/clewis/repos/ECE661/project/runs/classify/train2/weights/best.pt")
    ...
    def run_step(self):
        ...
        if data_id is not None:
            ...
            self.frame = self.frame[self.crop[2]:self.crop[3], self.crop[0]:self.crop[1]].astype(np.uint8)
            # have to make it 3 channels
            # self.frame = cv2.cvtColor(self.frame, cv2.COLOR_GRAY2BGR).astype(np.float32) / 255.0

            pred = self.model(self.frame, verbose=False)[0].probs.top1
            if pred == 1:
                LIFT_DETECTED = True
