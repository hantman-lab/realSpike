...
class CueGenerator(ZmqActor):
    ...
    def __str__(self):
        ...
        return f"Name: {self.name}, Experiment type: stim-only holography"
    ...
    def run_step(self):
        ...
        if buff is not None:
            try:
                ...
                self.improv_logger.info("STIM ONLY, SENT PATTERN SIGNAL")
                # wait for refresh
                time.sleep(0.015)

                self.laser_socket.send_string(f"{self.trial_num}")
                self.improv_logger.info("STIM ONLY, SENT LASER SIGNAL")
